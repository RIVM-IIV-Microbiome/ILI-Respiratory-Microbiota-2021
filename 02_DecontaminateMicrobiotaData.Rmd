...
#ps.total.public <- ps.total

#sample_data(ps.total.public) <- sample_data(total.meta)
#saveRDS(ps.total.public, "data/rds/ps.total.public.rds")
ps.total <- readRDS("data/rds/ps.total.public.rds")
...
01] ntaxa = 1039
02] nsamples = 118
03] nsamplesvariables = 17
04] nranks = 7
05] Min. number of reads = 108
06] Max. number of reads = 64780
07] Total number of reads = 2194212
08] Average number of reads = 18595.02
09] Median number of reads = 16858
10] Sparsity = 0.957651588065448
11] Number of singletons = 45
12] % of taxa that are singletons 
  (i.e. exactly one read detected across all samples) = 4.33108758421559

```{r}
plot_taxa_heatmap(ps.tech, 
                  transformation = "log10", 
                  subset.top = 50,
                  VariableA = c("control","control_source"),
                  cluster_cols = T,
                  show_colnames = F,
                  filename="controls/figs/control_source_heatmap.pdf",
                  height = 10, width = 14)
```
...
ps.sam <- subset_samples(ps.sam, is.na(control))
```
...
check for samples and controls together.
```{r}
ps.cont.sams <- ps.total
sample_data(ps.cont.sams)$control.2 <- ifelse(is.na(sample_data(ps.cont.sams)$control), "sample", sample_data(ps.cont.sams)$control)
oro.sams <- sample_names(subset_samples(ps.cont.sams, source =="oropharynx"))

ps.cont.sams <- prune_samples(!(sample_names(ps.cont.sams) %in% c(oro.sams,
                                                                  "071_330",
                                                                  "071_331",
                                                                  "075_067",
                                                                  "071_165")), ps.cont.sams)

plot_taxa_heatmap(ps.cont.sams, 
                  transformation = "log10", 
                  subset.top = 50,
                  VariableA = c("control.2","control_source"),
                  cluster_cols = T,
                  show_colnames = F,
                  filename="controls/figs/control_source_sampleheatmap.pdf",
                  height = 10, width = 14)
```
...
`ZymoMC` = 48 (Includes both gDNA and cells positive control)    
...
Among the top 50 are some ASVs that are prevalent and dominants in controls.  
...
```{r}
# order samples by a column/variable in R
blnk.dat <- meta(blnk.ps)
unique(blnk.dat$control_source)
blnk.dat <- blnk.dat %>% 
  arrange(control_source)
blnk.dat$control_source <- factor(blnk.dat$control_source, levels = c("Blank", "PCR blank"))
bln.otutab <- abundances(blnk.ps)
bln.otutab <- bln.otutab[,rownames(blnk.dat)]
otu_table(blnk.ps) <- otu_table(bln.otutab, taxa_are_rows = T)
sample_data(blnk.ps) <- sample_data(blnk.dat)

topOTU <- top_taxa(blnk.ps, n = 50)

library(RColorBrewer)

p.h <- plot_taxa_heatmap_2(blnk.ps, 
                           subset.top = 50, 
                           transformation = "log10",
                           VariableA = "control_source",
                           cluster_cols = FALSE)
annotation_row = data.frame(prevalence = p.h$prev)
...
p.h <- plot_taxa_heatmap(blnk.ps, 
                         ...
                         filename="controls/figs/blank_heatmap_prev.pdf",
                         height = 10, width = 14)
